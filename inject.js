...
var init = function init(_ref2) {
  ...
  var options = encodeURIComponent(
    JSON.stringify({ hideWidget: hideWidget, config: config })
  );
  var iframeSrc = host + "/lite/?m=channel-web&v=embedded&options=" + options;
  var iframeHTML =
    "<iframe id='bp-widget' frameborder='0' src='" + iframeSrc + "' />";
  injectDOMElement("div", "body", {
    id: "bp-web-widget",
    innerHTML: iframeHTML
  });

  var iframeWindow = document.querySelector("#bp-web-widget > #bp-widget")
    .contentWindow;
  var configure = function configure(payload) {
    return iframeWindow.postMessage(
      { action: "configure", payload: payload },
      "*"
    );
  };
  ...
  window.botpressWebChat = _extends({}, window.botpressWebChat, {
    configure: configure,
    sendEvent: sendEvent
  });
};
